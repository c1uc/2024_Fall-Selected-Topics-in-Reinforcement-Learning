        ...
        while self.total_time_step <= self.training_steps:
            ...
            while True:
                if self.total_time_step < self.warmup_steps:
                    action = self.decide_agent_actions(
                        observation, 1.0, self.env.action_space
                    )
                else:
                    action = self.decide_agent_actions(
                        observation, self.epsilon, self.env.action_space
                    )
                    self.epsilon_decay()
                ...
                if isinstance(terminate, bool):
                    ...
                    terminate = [int(term) for term in terminate]
                self.replay_buffer.append(
                    observation, [action], [reward], next_observation, [terminate]
                )
                ...
                if terminate or truncate:
                    ...
                    self.writer.add_scalar(
                        "Train/Episode Len", np.mean(episode_len), self.total_time_step
                    )
                    print(
    ...
    def evaluate(self):
        ...
        for i in range(self.eval_episode):
            ...
            while True:
                ...
                action = self.decide_agent_actions(
                    observation, self.eval_epsilon, self.test_env.action_space
                )
                next_observation, reward, terminate, truncate, info = (

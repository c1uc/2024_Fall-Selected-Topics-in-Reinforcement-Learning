...
class GaeSampleMemory(object):
    ...
    class Path(object):
        ...
        def append(self, sample):
            if (
                ...
                or self.trajectories[-1].transitions["done"][-1]
            ):
        ...
        def merge_observations(self, key):
            results = np.concatenate(
                [t.merge_observations(key) for t in self.trajectories]
            )
            return results
    ...
    def extract_batch(
        self, discount_gamma, discount_lambda, use_next_observation=False
    ):
        returns = []
        advs = []
        ...
        for key in self.paths[0].get_keys():
            if key == "observation":
                ...
                for obs_key in self.paths[0].get_observation_keys():
                    batchs[key][obs_key] = np.concatenate(
                        [
                            self.paths[i].merge_observations(obs_key)
                            for i in range(self.config["agent_count"])
                        ]
                    )
            else:
                batchs[key] = np.concatenate(
                    [
                        self.paths[i].merge(key)
                        for i in range(self.config["agent_count"])
                    ]
                )
        if use_next_observation:
            ...
            for obs_key in self.paths[0].get_observation_keys():
                batchs["next_observation"][obs_key] = np.concatenate(
                    [
                        self.paths[i].merge_next_observations(obs_key)
                        for i in range(self.config["agent_count"])
                    ]
                )
    ...
    def _compute_discounted_return(
        self, rewards, dones, discount_gamma, discount_lambda
    ):

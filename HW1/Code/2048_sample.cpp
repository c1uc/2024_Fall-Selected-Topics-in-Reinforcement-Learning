...
public:
	...
	virtual float update(const board& b, float u) {
		// TODO
        float u_split = u / (float)isomorphic.size();
        float val = 0.0;
        for(const auto& iso : isomorphic) {
            size_t index = indexof(iso, b);
...
public:
			...
			if (move->assign(b)) {
                ...
                for(int i = 0;i < 16;i++) {
                    if(move->after_state().at(i) == 0) {
                        ...
                        temp.set(i, 1);
                        total_value += 0.9f * (float)estimate(temp);
                        temp.set(i, 2);
                        total_value += 0.1f * (float)estimate(temp);
                    }
                }

                total_value /= (float)total_space;
                total_value += (float)move->reward();
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
        ...
        for(auto it1 = ++path.rbegin(); it1 != path.rend(); ++it1) {
            float err = (float)it1->reward() + next_est - estimate(it1->before_state());
            next_est = update(it1->before_state(), alpha * err);
        }
	}
...
//    tdl.load("");
    ...
    // use a vector to store the value of each episode
	...
	for (size_t n = 1; n <= total; n++) {
        ...
        // store the value of each episode
        mean_values.push_back(score);
	}
...
//	tdl.save("./model1.bin");
    ...
    // output the value of each episode
//    std::ofstream out("value.txt");
//    for(auto value : mean_values) {
//        out << value << std::endl;
//    }
//    out.close();
